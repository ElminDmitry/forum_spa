class Auth::SessionsController < Devise::SessionsController
  ...
  def create
    # render json: { msg: 'Проверка каптчи не пройдена' }, status: 422 and return unless verify_captcha(params[:user][:recaptcha])
    # self.resource = warden.authenticate(auth_options)
    # render json: { msg: 'Email не найден, либо пароль неверен' }, status: 401 and return unless self.resource
